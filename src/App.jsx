...
function App() {
  ...
  const API_KEY = import.meta.env.VITE_API_KEY; // Replace with your OpenWeather API key
  ...
  const handleSearch = () => {
    if (input.trim()) {
      fetchWeatherData(input); // Fetch data only when the search button is clicked
    } else {
  ...
  return (
    <>
      <div className='h-full max-w-dvw bg-slate-800 dark:bg-slate-100 p-4 duration-300 transition-all'> 
        <Navigation />
        ...
        {weatherData && (
          <div className='mt-12 grid grid-cols-3 justify-items-stretch gap-2'>
            <div className='col-span-3 md:col-span-2 md:p-6'>
              <div className='flex md:grid md:grid-cols-2 justify-content-between'>
              ...
              <div className='rounded-xl mt-6 bg-slate-600 dark:bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90% p-4 min-w-[270px] overflow-y-scroll md:overflow-hidden'>
              ...
              <div className='rounded-xl mt-6 bg-slate-600 dark:bg-green-500 p-4'>
            ...
            <div className='h-min col-span-3 md:col-span-1 p-4 mt-4 md:mt-0 rounded-xl bg-slate-600 dark:bg-slate-200'>
